...
[[nodiscard]] NeutralOptional<NeutralString> parse_line_style_name(NeutralStringView instruction) {
    const auto line_style_begin = instruction.find("LS(");
    if(line_style_begin == NeutralStringView::npos) {
        return std::nullopt;
    }

    const auto name_begin = line_style_begin + 3;
    const auto name_end = instruction.find_first_of(",)", name_begin);
    if(name_end == NeutralStringView::npos) {
        return std::nullopt;
    }

    const auto line_style_name = make_neutral_string(instruction.substr(name_begin, name_end - name_begin));
...
NeutralOptional<LinePortrayalSelection> select_line_portrayal(
    const LookupIndex& lookup_index,
    const RuleLayerFeature& feature,
    const MarinerSettings& mariner_settings) {
    if(feature.primitive_type != FeaturePrimitiveType::line) {
        return std::nullopt;
    }
    ...
    LinePortrayalSelection selection;
    selection.lookup_key = *lookup_key;
    selection.lookup_entry = bucket->entries.front();
    selection.line_style_name = parse_line_style_name(selection.lookup_entry.instruction).value_or("");
